...
void main(void) {
    // Инициализация регистров системного управления.
    InitSysCtrl();

    // Инициализация GPIO-пинов для работы в режиме ePWM.
    InitEPwm1Gpio();
    InitEPwm2Gpio();
    InitEPwm3Gpio();

    // Отключение прерываний ЦПУ.
    DINT;

    // Инициализация регистров управления PIE. Отключение прерываний о сброс флагов.
    InitPieCtrl();

    // Отключение перываний ЦПУ и сброс флагов ЦПУ.
    IER = 0x0000;
    IFR = 0x0000;
    ...
    // Включение прерываний ЦПУ, для группы M_INT3, куда подключены ePWM1-3 INT.
    IER |= M_INT3;

    // Включение прерываний ePWM в PIE
    PieCtrlRegs.PIEIER3.bit.INTx1 = 1;

    // Включение глобальных прерываний и установка наивысшего приоритета для событий отладки реального времени.
    EINT;
    ERTM;
...
__interrupt void epwm1_isr(void) {
    EPwm1Regs.CMPA.half.CMPA = 3750 + ((sin_table[idx] - 3750) * 1);
    EPwm2Regs.CMPA.half.CMPA = 3750 + ((sin_table[(idx + N/3) % N] - 3750) * m);
    // EPwm3Regs.CMPA.half.CMPA = 3750 + ((sin_table[(idx + 2*N/3) % N] - 3750) * m);
